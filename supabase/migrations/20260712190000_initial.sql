-- Controle de Pedidos Kanban — estrutura inicial segura e repetível para Supabase
-- Migration base com timestamp válido. Pode concluir uma Preview parcialmente criada
-- sem recriar tipos, tabelas, índices, gatilhos ou políticas já existentes.
...
create index if not exists orders_board_idx
  on public.orders (sector_id, status, position);
create index if not exists orders_delivery_idx
  on public.orders (delivery_date) where completed_at is null;
