...
import { menuItems, priorityLabel, roleLabel, statusToDb } from "@/lib/pcp-config";
import {
  dateTimeLabel,
  dueLabel,
  initials,
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (dbError) {
        ...
        const storedImages = loadedOrders.filter((order) => order.main_image_path && !/^https?:\/\//i.test(order.main_image_path));
        loadedOrders.forEach((order) => {
          if (order.main_image_path && /^https?:\/\//i.test(order.main_image_path)) nextImageUrls[order.id] = order.main_image_path;
        });
    ...
    const channel = supabase.channel("orders-board")
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "order_comments" }, () => void load())
  ...
  async function saveOrderSummary(event: FormEvent<HTMLFormElement>, order: Order) {
    ...
    await updateOrder(order, {
      op_number: String(form.get("op_number") || "").trim(),
  ...
  async function saveProductionDetails(event: FormEvent<HTMLFormElement>, order: Order) {
    ...
    const form = new FormData(event.currentTarget);
    await updateOrder(order, {
      sector_id: String(form.get("sector_id") || order.sector_id),
      status: String(form.get("status") || order.status) as DbStatus,
  ...
  async function removeOrderFileLink(file: OrderFileEntry) {
    if (!isAdmin || !user || workspaceBusy || !window.confirm(`Remover ${file.file_name} somente desta OS? O arquivo continuará no Google Drive.`)) return;
    setWorkspaceBusy(true);
    setDetailError("");
    ...
    if (recordError) {
      ...
      showNotice("Arquivo removido da OS. O conteúdo foi mantido no Google Drive e não voltará ao usar Atualizar arquivos.");
    }
    setWorkspaceBusy(false);
  }
  ...
  async function deleteOrder(order: Order) {
    ...
    setBusyOrderId(order.id);
    setError("");
    const { data: deletedOrder, error: deleteError } = await supabase.from("orders").delete().eq("id", order.id).select("id").maybeSingle();
    if (deleteError || !deletedOrder) {
      setError(`Não foi possível apagar o pedido: ${deleteError?.message || "operação não autorizada"}`);
      setBusyOrderId(null);
      return;
    }
  ...
  async function createUser(event: FormEvent<HTMLFormElement>) {
    ...
    setUserCreateError("");
    const { data, error: functionError } = await supabase.functions.invoke<CreateUserResponse>("admin-create-user", {
      body: { name, email, role },
    });

    let functionMessage = data?.error || functionError?.message || "";
    const context = (functionError as unknown as { context?: Response } | null)?.context;
    if (context) {
      try {
        const payload = await context.clone().json() as CreateUserResponse;
        functionMessage = payload.error || functionMessage;
      } catch {
        // Mantém a mensagem genérica quando o serviço não retorna JSON.
      }
    }

    if (functionError || !data?.ok || !data.user) {
      setUserCreateError(functionMessage || "Não foi possível criar o usuário. Tente novamente.");
      setCreatingUser(false);
      return;
    }

    setProfiles((current) => [...current.filter((profile) => profile.id !== data.user?.id), data.user as Profile]
      .sort((a, b) => (a.name || a.email).localeCompare(b.name || b.email, "pt-BR")));
    formElement.reset();
    setUserModalOpen(false);
    setCreatingUser(false);
    showNotice("Convite enviado para " + data.user.email + ".");
  }
  ...
  async function addOrders(submission: OrderBatchSubmission) {
    ...
    const normalizedNewOps = submission.items.map((item) => item.opNumber.trim().toLocaleUpperCase("pt-BR"));
    ...
    const alreadyExists = submission.items.find((item) => existingOps.has(item.opNumber.trim().toLocaleUpperCase("pt-BR")));
    if (alreadyExists) {
      setError(`A OP ${alreadyExists.opNumber} já está cadastrada.`);
      return;
    }

    const missingClientItem = submission.items.find((item) => !clients.some((client) => client.id === item.clientId));
    if (missingClientItem) {
      setError(`Selecione um cliente válido para a OP ${missingClientItem.opNumber}.`);
      return;
    }

    const payloads = submission.items.map((item) => {
      const selectedClient = clients.find((client) => client.id === item.clientId) as Client;
      return {
    ...
    if (insertError || !createdOrders) {
      const duplicateMessage = insertError?.code === "23505" ? "Um dos números de OP já existe no sistema." : insertError?.message;
      setError(`Não foi possível cadastrar ${submission.mode === "batch" ? "os subpedidos" : "o pedido"}: ${duplicateMessage || "resposta inválida do banco"}`);
    ...
    await Promise.all(submission.items.map(async (item) => {
      ...
      const imagePath = `orders/${orderId}/${crypto.randomUUID()}.png`;
    ...
    showNotice(submission.mode === "batch" ? `${submission.items.length} subpedidos cadastrados na OP ${submission.baseOp}.` : "Pedido cadastrado com sucesso.");
  }
  ...
  if (authMode === "update" || !user) return <main className="login-shell">
    ...
    <section className="login-panel">{authMode === "update" ? <form className="login-card" onSubmit={updatePassword}>
      ...
      <button className="primary login-button" disabled={authBusy}>{authBusy ? "Salvando…" : "Salvar nova senha"}</button>
      ...
      <button className="primary login-button" disabled={authBusy}>{authBusy ? "Enviando…" : "Enviar link de recuperação"}</button>
      ...
      <button className="primary login-button" disabled={authBusy}>{authBusy ? "Entrando…" : "Entrar"}</button>
  ...
  return <main className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <section className="content">
      ...
      {activeView === "dashboard" && <section className="v3-dashboard">
        <div className="v3-hero">
          ...
          {canOperate && <button className="primary" onClick={() => openNewOrder()}>＋ Nova ordem</button>}
        </div>
        ...
        <div className="v3-dashboard-grid">
          <article className="v3-panel">
            <header><div><span>CAPACIDADE</span><h3>Pedidos por setor</h3></div><button onClick={() => navigateTo("kanban")}>Abrir Kanban →</button></header>
      ...
      <section className="toolbar"><label className="search-field">⌕<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por OP, cliente ou serviço…" />{search && <button className="search-clear" aria-label="Limpar busca" onClick={() => setSearch("")}>×</button>}</label><button className={filtersOpen ? "active" : ""} onClick={() => setFiltersOpen((value) => !value)} aria-expanded={filtersOpen}>☷ Filtros {activeFilterCount > 0 && <b>{activeFilterCount}</b>}</button><button className="sort-button" onClick={cycleSortMode} title="Clique para alterar a ordenação">↕ {sortLabel[sortMode]}</button><span>{loading ? "Atualizando…" : `${filtered.length} pedido${filtered.length === 1 ? "" : "s"} encontrado${filtered.length === 1 ? "" : "s"}`}</span></section>
      {filtersOpen && <>
        <button className="filters-backdrop" aria-label="Fechar filtros" onClick={() => setFiltersOpen(false)} />
        <section className="filters-panel" aria-label="Filtros do Kanban">
          ...
          <label>Status<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as "all" | DbStatus)}><option value="all">Todos os status</option><option value="waiting">Aguardando</option><option value="in_progress">Em andamento</option></select></label>
          ...
          <button className="clear-filters" onClick={clearFilters} disabled={!activeFilterCount && !search}>Limpar busca e filtros</button>
      ...
      >
        {visibleSectors.map((sector) => <article className="sector" key={sector.id}>
          ...
          {(["Aguardando", "Em andamento"] as UiStatus[]).map((status) => <div className={`lane ${dragOverLane === `${sector.id}:${status}` ? "drag-over" : ""}`} key={status} aria-label={`${sector.name} — ${status}`} onDragOver={(event) => { if (!canOperate) return; event.preventDefault(); event.dataTransfer.dropEffect = "move"; setDragOverLane(`${sector.id}:${status}`); }} onDrop={() => void move(sector.id, status)}>
            <div className="lane-head"><b><i className={status === "Aguardando" ? "dot wait" : "dot run"} />{status}</b><span>{filtered.filter((o) => o.sector_id === sector.id && o.status === statusToDb[status]).length}</span></div>
            {filtered.filter((o) => o.sector_id === sector.id && o.status === statusToDb[status]).map((order) => <div draggable={canOperate && busyOrderId !== order.id} aria-disabled={!canOperate} onDragStart={(event) => { if (!canOperate) { event.preventDefault(); return; } event.dataTransfer.effectAllowed = "move"; setDragged(order.id); }} onDragEnd={() => { setDragged(null); setDragOverLane(null); }} onClick={() => openOrder(order, "history")} className={`order ${priorityLabel[order.priority].toLowerCase()}`} key={order.id}>
              ...
              <div className="order-thumbnail">{cardImageUrl(order) ? <img src={cardImageUrl(order)} alt={`Miniatura da OP ${order.op_number}`} width={160} height={160} loading="lazy" /> : <div className="order-thumbnail-empty" aria-label="Pedido sem miniatura"><span>PNG</span><small>Sem miniatura</small></div>}</div>
        ...
        {!loading && hasActiveSearch && !filtered.length && <div className="search-empty"><span>⌕</span><b>Nenhum pedido encontrado</b><p>Tente alterar o termo pesquisado ou limpar os filtros.</p><button onClick={clearFilters}>Limpar busca e filtros</button></div>}
    ...
    {modal && <div className="overlay" onMouseDown={() => { if (!creatingOrder) { setNewOrderInitialClientId(""); setModal(null); } }}><div className={`modal ${modal === "new" ? "order-create-modal" : ""}`} onMouseDown={(e) => e.stopPropagation()}><button className="close" disabled={creatingOrder} onClick={() => { setNewOrderInitialClientId(""); setModal(null); }}>×</button>
      ...
      /></> : <>
        <div className="v3-order-header">
          <div className="v3-order-image">{cardImageUrl(modal) ? <img src={cardImageUrl(modal)} alt={`Miniatura da OP ${modal.op_number}`} /> : <span>PNG<br />Sem miniatura</span>}</div>
        ...
        <div className="detail-tabs v3-detail-tabs">
          <button className={detailTab === "summary" ? "active" : ""} onClick={() => setDetailTab("summary")}>Resumo</button>
          <button className={detailTab === "production" ? "active" : ""} onClick={() => setDetailTab("production")}>Produção</button>
          <button className={detailTab === "materials" ? "active" : ""} onClick={() => setDetailTab("materials")}>Materiais <span>{materials.length}</span></button>
          <button className={detailTab === "files" ? "active" : ""} onClick={() => setDetailTab("files")}>Arquivos <span>{orderFiles.length}</span></button>
          <button className={detailTab === "checklist" ? "active" : ""} onClick={() => setDetailTab("checklist")}>Checklist <span>{checklist.filter((item) => item.completed).length}/{checklist.length}</span></button>
          <button className={detailTab === "installation" ? "active" : ""} onClick={() => setDetailTab("installation")}>Instalação/entrega</button>
          <button className={detailTab === "history" ? "active" : ""} onClick={() => setDetailTab("history")}>Histórico</button>
          <button className={detailTab === "comments" ? "active" : ""} onClick={() => setDetailTab("comments")}>Comentários <span>{commentCounts[modal.id] || 0}</span></button>
        </div>
        ...
        {detailLoading ? <div className="detail-loading">Carregando informações…</div> : detailTab === "summary" ? <form className="v3-order-summary editable-order-form" onSubmit={(event) => void saveOrderSummary(event, modal)}>
          <div className="form-grid">
            <label>Número da OP<input name="op_number" defaultValue={modal.op_number} required /></label>
          ...
          {canOperate && <div className="actions"><button className="primary" disabled={busyOrderId === modal.id}>{busyOrderId === modal.id ? "Salvando…" : "Salvar alterações"}</button></div>}
          ...
          <div className="form-grid">
            ...
            <label>Status<select name="status" defaultValue={modal.status}><option value="waiting">Aguardando</option><option value="in_progress">Em andamento</option><option value="paused">Pausado</option><option value="completed">Concluído</option></select></label>
          </div>
          <div className="v3-production-actions">{canOperate && <button className="primary" disabled={busyOrderId === modal.id}>{busyOrderId === modal.id ? "Salvando…" : "Salvar produção"}</button>}{canOperate && modal.status !== "completed" && <button type="button" onClick={() => void finishOrder(modal)}>Finalizar ordem</button>}</div>
          ...
          <div className="drive-guidance"><b>Organização automática no Drive</b><span>O sistema cria a estrutura Cliente → OP → Subpedido → Arte, Produção, Documentos, Fotos e Instalação. Os arquivos permanecem no Google Drive da conta conectada.</span></div>
          <OrderDriveUpload
            orderId={modal.id}
          ...
          {canUploadFiles && <details className="drive-manual-link"><summary>Vincular um link existente do Google Drive manualmente</summary><form className="drive-link-form" onSubmit={registerDriveFile}>
            ...
            <button className="primary" disabled={workspaceBusy}>{workspaceBusy ? "Salvando…" : "Vincular link"}</button>
          </form></details>}
          <div className="os-file-grid">{orderFiles.map((file) => {
            ...
            return <article key={file.id} className={`os-file-card drive-file-card ${file.is_approved ? "approved" : ""}`}>
              ...
              <div className="file-card-content"><b>{file.file_name}</b><small>{file.version ? `${file.version} · ` : ""}{file.file_category === "art" ? "Arte" : file.file_category === "approval" ? "Aprovação" : file.file_category === "production" ? "Produção" : file.file_category === "photo" ? "Fotos" : file.file_category === "installation" ? "Instalação" : file.file_category === "document" ? "Documento" : "Outro"} · {dateTimeLabel(file.created_at)}</small>{file.notes && <p>{file.notes}</p>}{file.is_approved && <em>ARTE APROVADA</em>}
                <div className="file-audit-info"><span><strong>{originLabel}</strong> {addedBy?.name || addedBy?.email || "Usuário não identificado"}</span>{updatedBy && <span><strong>Última sincronização no Publicolor:</strong> {updatedBy.name || updatedBy.email} · {dateTimeLabel(file.updated_at)}</span>}{driveEditor && <span><strong>Última alteração no Google Drive:</strong> {driveEditor}{file.drive_modified_at ? ` · ${dateTimeLabel(file.drive_modified_at)}` : ""}</span>}</div>
              </div>
              <div className="file-actions">{file.drive_file_id && <button type="button" onClick={() => void downloadOrderFile(file)} disabled={workspaceBusy}>Baixar</button>}{file.drive_url && <a href={file.drive_url} target="_blank" rel="noreferrer">Abrir no Drive</a>}{isAdmin && <button className="file-unlink-button" type="button" onClick={() => void removeOrderFileLink(file)} disabled={workspaceBusy}>Remover da OS</button>}{isAdmin && file.drive_file_id && <button className="file-delete-drive-button" type="button" onClick={() => void deleteOrderFileFromDrive(file)} disabled={workspaceBusy}>Excluir do Drive</button>}</div>
            </article>;
          })}</div>
          ...
          {!checklist.length && canOperate && <button className="checklist-template-button" onClick={() => void createDefaultChecklist()} disabled={workspaceBusy}>Criar checklist padrão da Publicolor</button>}
          ...
          {canOperate && <form className="checklist-add-form" onSubmit={addChecklistItem}><input name="label" placeholder="Novo item de conferência" required /><input name="category" placeholder="Categoria" defaultValue="Geral" /><button className="primary" disabled={workspaceBusy}>Adicionar item</button></form>}
          ...
          <div className="actions"><button type="button" onClick={() => navigateTo("installation")}>Abrir agenda</button>{canOperate && <button className="primary" disabled={busyOrderId === modal.id}>{busyOrderId === modal.id ? "Salvando…" : "Salvar instalação"}</button>}</div>
          ...
          <form className="comment-form" onSubmit={addComment}><textarea name="comment" placeholder="Adicionar comentário sobre este pedido…" maxLength={4000} required /><button className="primary" disabled={commentSending}>{commentSending ? "Enviando…" : "Enviar comentário"}</button></form>
        </div>}
      </>}
    </div></div>}
    {reopenOrderTarget && <div className="overlay" onMouseDown={() => { if (!reopeningOrder) setReopenOrderTarget(null); }}>
      <div className="modal reopen-order-modal" role="dialog" aria-modal="true" aria-labelledby="reopen-order-title" onMouseDown={(event) => event.stopPropagation()}>
        <button className="close" aria-label="Fechar reabertura do pedido" disabled={reopeningOrder} onClick={() => setReopenOrderTarget(null)}>×</button>
        ...
        <form className="reopen-order-form" onSubmit={reopenCompletedOrder}>
          ...
          <div className="actions">
            ...
            <button className="primary" disabled={reopeningOrder}>{reopeningOrder ? "Devolvendo…" : "Confirmar retorno"}</button>
          </div>
        </form>
      </div>
    </div>}
    ...
    {userModalOpen && isAdmin && <div className="overlay" onMouseDown={() => { if (!creatingUser) setUserModalOpen(false); }}>
      <div className="modal user-invite-modal" role="dialog" aria-modal="true" aria-labelledby="new-user-title" onMouseDown={(event) => event.stopPropagation()}>
        <button className="close" aria-label="Fechar cadastro de usuário" disabled={creatingUser} onClick={() => setUserModalOpen(false)}>×</button>
        ...
        <form className="user-invite-form" onSubmit={createUser}>
          ...
          <div className="actions"><button type="button" disabled={creatingUser} onClick={() => setUserModalOpen(false)}>Cancelar</button><button className="primary" disabled={creatingUser}>{creatingUser ? "Enviando convite…" : "Enviar convite"}</button></div>
        </form>
      </div>
    </div>}
  </main>;
