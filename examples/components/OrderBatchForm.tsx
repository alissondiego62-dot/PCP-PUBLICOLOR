...
export function OrderBatchForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalizedBaseOp) {
      setLocalError("Informe o número da OP.");
      return;
    }
    ...
    await onSubmit({
      ...
      items: selectedItems.map((item) => ({
        opNumber: mode === "batch" ? `${normalizedBaseOp}-${item.suffix.trim()}` : normalizedBaseOp,
    ...
    <form className="order-batch-form" onSubmit={submit}>
      ...
      <section className="shared-order-data">
        ...
        <div className="form-grid order-shared-grid">
          <label>Número da OP principal<input value={baseOp} onChange={(event) => setBaseOp(event.target.value)} placeholder="Ex.: 959" required /></label>
      ...
      <div className="actions order-batch-actions">
        ...
        <button className="primary" disabled={busy}>{busy ? "Cadastrando…" : mode === "batch" ? `Cadastrar ${visibleItems.length} subpedidos` : "Cadastrar pedido"}</button>
      </div>
    </form>
  );
}
