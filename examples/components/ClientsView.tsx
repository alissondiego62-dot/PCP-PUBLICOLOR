...
export function ClientsView({ clients, orders, canOperate, onOpenOrder, onNewClient, onEditClient, onNewOrder }: Props) {
  ...
  const selectedOrders = selected ? orders.filter((order) => order.client_id === selected.id || (!order.client_id && order.client_name === selected.name)) : [];
    ...
    return (
      <section className="client-detail-view">
        <div className="client-detail-header">
          ...
          {canOperate && <div className="client-detail-actions"><button type="button" onClick={() => onEditClient(selected)}>Editar cadastro</button><button className="primary" onClick={() => onNewOrder(selected)}>＋ Novo pedido</button></div>}
        </div>
  ...
  return (
    <section className="management-view clients-management-view">
      <div className="view-toolbar">
        ...
        {canOperate && <button className="primary" onClick={onNewClient}>＋ Novo cliente</button>}
      </div>
      ...
      <div className="clients-cards-grid">
        {filtered.map((client) => {
          const clientOrders = orders.filter((order) => order.client_id === client.id || (!order.client_id && order.client_name === client.name));
