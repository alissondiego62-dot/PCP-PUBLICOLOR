...
export async function listDriveFilesForOrder(order: { id: string; op_number: string; client_name: string }) {
  const orderFolder = await findOrderFolder(order);
  if (orderFolder) {
    return collectOrderFolderFiles(orderFolder.id);
  }

  // Compatibilidade com arquivos enviados antes da navegação por pasta.
  const params = new URLSearchParams({
    q: [
      "trashed = false",
