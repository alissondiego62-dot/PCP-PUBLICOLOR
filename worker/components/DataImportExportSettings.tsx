...
export function DataImportExportSettings({ orders, clients, sectors, onImportComplete }: Props) {
  ...
  async function importOrders(): Promise<ImportReport> {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      const row = rows[index];
      const line = index + 2;
      const opNumber = pick(row, orderAliases.opNumber).toLocaleUpperCase("pt-BR");
      if (!opNumber) {
        result.errors.push(`Linha ${line}: número da OP não informado.`);
        continue;
      }
      ...
      if (existing) {
        ...
        if (error || !data) {
          result.errors.push(`Linha ${line} (OP ${opNumber}): ${error?.message || "falha ao atualizar"}.`);
          continue;
        }
        result.updated += 1;
      } else {
        ...
        if (error || !data) {
          result.errors.push(`Linha ${line} (OP ${opNumber}): ${error?.message || "falha ao criar"}.`);
          continue;
        }
