...
export function OrderDriveUpload({ orderId, opNumber, canOperate, onUploaded, onError, onNotice }: Props) {
  ...
  async function reconcileOrderFiles() {
    ...
    try {
      ...
      let message = "";
      if (result.linked > 0 || result.updated > 0 || result.restored > 0) {
        const parts: string[] = [];
        if (result.linked > 0) parts.push(`${result.linked} novo(s) arquivo(s) vinculado(s)`);
        if (result.restored > 0) parts.push(`${result.restored} arquivo(s) restaurado(s) na OS`);
        const metadataUpdates = Math.max(0, result.updated - result.restored);
        if (metadataUpdates > 0) parts.push(`${metadataUpdates} arquivo(s) atualizado(s)`);
        message = `${parts.join(" e ")} na OP ${opNumber}.`;
      } else if (result.found > 0) {
        message = `Sincronização concluída. Os ${result.found} arquivo(s) localizados já estavam atualizados.`;
      } else {
        message = "Nenhum arquivo foi localizado na pasta desta ordem.";
      }
