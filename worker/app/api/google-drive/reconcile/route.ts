...
export async function POST(request: Request) {
  try {
    ...
    const [{ data: folderRows, error: folderError }, { data: pendingRows, error: pendingError }] = await Promise.all([
      admin
        .from("order_files")
    ...
    if (folderError) throw new Error(`Não foi possível localizar as pastas já vinculadas à ordem: ${folderError.message}`);
    if (pendingError) throw new Error(`Não foi possível consultar os envios pendentes: ${pendingError.message}`);

    const knownFolders = [
      ...(folderRows || []).map((row: { drive_folder_id: string | null; file_category: string | null }) => ({
        id: row.drive_folder_id || "",
        category: row.file_category || "other",
      })),
      ...(pendingRows || []).map((row: { drive_folder_id: string; file_category: string }) => ({
        id: row.drive_folder_id,
        category: row.file_category || "other",
      })),
    ].filter((folder) => folder.id);

    const driveFiles = await listDriveFilesForOrder(order, knownFolders);
    const scannedFolders = new Set(knownFolders.map((folder) => folder.id)).size;
    if (!driveFiles.length) {
      return Response.json({ ok: true, linked: 0, updated: 0, restored: 0, found: 0, scanned_folders: scannedFolders });
    }

    const ids = driveFiles.map((file) => file.id);
    const { data: existingRows, error: existingError } = await admin
      .from("order_files")
      .select("id,drive_file_id,file_name,file_type,file_size,drive_url,drive_folder_id,file_category,drive_modified_at,drive_last_modified_by_name,drive_last_modified_by_email,drive_md5_checksum,removal_mode,removed_from_order_at,removed_from_order_by")
      .eq("order_id", orderId)
    ...
    return Response.json({ ok: true, linked, updated, restored, found: driveFiles.length, scanned_folders: scannedFolders });
  } catch (error) {
