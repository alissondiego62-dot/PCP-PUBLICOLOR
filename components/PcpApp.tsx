...
const CompletedOrdersView = lazy(() => import("@/components/CompletedOrdersView").then((module) => ({ default: module.CompletedOrdersView })));
const InstallationAgendaView = lazy(() => import("@/app/components/InstallationAgendaView").then((module) => ({ default: module.InstallationAgendaView })));
const ActivitiesView = lazy(() => import("@/components/ActivitiesView").then((module) => ({ default: module.ActivitiesView })));
const ClientsView = lazy(() => import("@/components/ClientsView").then((module) => ({ default: module.ClientsView })));
const UsersView = lazy(() => import("@/features/users/UsersView").then((module) => ({ default: module.UsersView })));
...
export function PcpApp({ initialView = "dashboard" }: { initialView?: ViewKey }) {
  ...
  const [thumbnailBackgroundMode, setThumbnailBackgroundMode] = useState<"always" | "wifi" | "visible_only">("wifi");
  const [sortMode, setSortMode] = useState<SortMode>("newest");
  const [activeView, setActiveView] = useState<ViewKey>(initialView);
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [globalSearchOpen, setGlobalSearchOpen] = useState(false);
  const [pendingCenterOpen, setPendingCenterOpen] = useState(false);
  ...
  const detailLoadedTabsRef = useRef<Record<string, Set<DetailTab>>>({});
  ...
  const restoringViewFiltersRef = useRef(false);

  useEffect(() => {
    restoringViewFiltersRef.current = true;
    const saved = window.sessionStorage.getItem(`pcp-view-state:${activeView}`);
    if (saved) {
      try {
        ...
        setResponsibleFilter(state.responsibleFilter || "all");
        setSectorFilter(state.sectorFilter || "all");
        setStatusFilter(state.statusFilter || "all");
        setPriorityFilter(state.priorityFilter || "all");
        setDeadlineFilter(state.deadlineFilter || "all");
        setSortMode(state.sortMode || "newest");
      } catch {
        window.sessionStorage.removeItem(`pcp-view-state:${activeView}`);
      }
    } else {
      const query = new URLSearchParams(window.location.search);
      setSearch(query.get("busca") || "");
      setResponsibleFilter(query.get("responsavel") || "all");
      setSectorFilter(query.get("setor") || "all");
      setStatusFilter((query.get("status") as "all" | DbStatus) || "all");
      setPriorityFilter((query.get("prioridade") as "all" | Priority) || "all");
      setDeadlineFilter((query.get("prazo") as DeadlineFilter) || "all");
      setSortMode((query.get("ordem") as SortMode) || "newest");
    }
    window.requestAnimationFrame(() => { restoringViewFiltersRef.current = false; });
  }, [activeView]);

  useEffect(() => {
    if (restoringViewFiltersRef.current) return;
    window.sessionStorage.setItem(`pcp-view-state:${activeView}`, JSON.stringify({
      search,
      responsibleFilter,
      sectorFilter,
      statusFilter,
    ...
    const query = new URLSearchParams(window.location.search);
    if (search.trim()) query.set("busca", search.trim()); else query.delete("busca");
    if (responsibleFilter !== "all") query.set("responsavel", responsibleFilter); else query.delete("responsavel");
    if (sectorFilter !== "all") query.set("setor", sectorFilter); else query.delete("setor");
    if (statusFilter !== "all") query.set("status", statusFilter); else query.delete("status");
    if (priorityFilter !== "all") query.set("prioridade", priorityFilter); else query.delete("prioridade");
    if (deadlineFilter !== "all") query.set("prazo", deadlineFilter); else query.delete("prazo");
    if (sortMode !== "newest") query.set("ordem", sortMode); else query.delete("ordem");
    const nextUrl = `${window.location.pathname}${query.size ? `?${query.toString()}` : ""}`;
    window.history.replaceState({ view: activeView }, "", nextUrl);
  }, [activeView, search, responsibleFilter, sectorFilter, statusFilter, priorityFilter, deadlineFilter, sortMode]);
  ...
  useEffect(() => {
    ...
    const lastSeenKey = `pcp-last-seen:${user.id}`;
    const previous = Number(window.sessionStorage.getItem(lastSeenKey) || 0);
    if (Date.now() - previous < 10 * 60 * 1000) return;
    window.sessionStorage.setItem(lastSeenKey, String(Date.now()));
    void supabase.rpc("touch_my_profile");
  }, [user?.id, isOnline]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const needsOrders = !["users", "settings", "completed"].includes(activeView);
      const needsClients = activeView === "orders" || activeView === "clients";
      const needsCommentCounts = activeView === "kanban";
      const needsCompleted = activeView === "clients" || activeView === "dashboard";
      const emptyResult = Promise.resolve({ data: [], error: null });
      const activeOrdersQuery = needsOrders
        ? supabase.from("orders").select(orderColumns).neq("status", "completed").order("position").limit(1000)
        : emptyResult;
      const completedOrdersQuery = needsCompleted
        ? (() => {
            let query = supabase.from("orders").select(orderColumns).eq("status", "completed").order("completed_at", { ascending: false, nullsFirst: false });
            if (activeView === "dashboard") {
              const sevenDaysAgo = new Date(Date.now() - (7 * 86400000)).toISOString();
              query = query.gte("completed_at", sevenDaysAgo);
            }
            return query.limit(activeView === "dashboard" ? 1000 : 500);
          })()
        : emptyResult;
      ...
      const orderResult = {
        data: [...(activeOrderResult.data || []), ...(completedOrderResult.data || [])],
        error: activeOrderResult.error || completedOrderResult.error,
      };
    ...
    void load();
    return () => { active = false; };
  }, [user, reloadToken, isOnline, activeView]);
  ...
  }, [modal, detailTab, isOnline]);

  useEffect(() => {
    if (!user || !isOnline || !modal || modal === "new" || detailTab !== "materials") return;
  ...
  const immediatePendingCount = useMemo(() => activeOrders.filter((order) =>
    order.blocked
    || order.status === "paused"
    || !order.consultant_name?.trim()
    || dueLabel(order.delivery_date).startsWith("Atrasado"),
  ).length, [activeOrders]);
  ...
  useEffect(() => {
    if (!user?.id || !isOnline) return;
    void supabase.from("operational_settings").select("setting_value").eq("setting_key", "thumbnail_background_mode").maybeSingle().then(({ data }) => {
      const value = String(data?.setting_value || "wifi");
      if (["always", "wifi", "visible_only"].includes(value)) setThumbnailBackgroundMode(value as "always" | "wifi" | "visible_only");
    });
  }, [user?.id, isOnline]);

  useEffect(() => {
    const connection = (navigator as Navigator & { connection?: { saveData?: boolean; effectiveType?: string; type?: string } }).connection;
    const restrictedConnection = thumbnailBackgroundMode === "wifi" && Boolean(connection?.saveData || ["slow-2g", "2g", "3g"].includes(connection?.effectiveType || ""));
    if (activeView !== "kanban" || !user || !isOnline || thumbnailBackgroundMode === "visible_only" || restrictedConnection) {
  ...
  }, [activeOrders, activeSectors, activeView, isOnline, loadOrderThumbnail, thumbnailBackgroundMode, user?.id]);
  ...
  usePcpRealtime({
    ...
    scope: {
      orders: ["kanban", "orders", "installation", "activities", "clients"].includes(activeView),
      comments: activeView === "kanban" || Boolean(modal && modal !== "new" && detailTab === "comments"),
      profiles: activeView === "users",
      clients: activeView === "clients" || activeView === "orders",
      sectors: ["kanban", "orders", "installation", "settings"].includes(activeView),
    },
  ...
  const viewMeta: Record<ViewKey, { eyebrow: string; title: string; description: string }> = {
    ...
    activities: { eyebrow: "ATIVIDADES E COMPRAS", title: "Atividades e Compras", description: "Organize tarefas, materiais, preços e compras vinculadas às ordens de serviço." },
  ...
  function openOrder(order: Order, tab: DetailTab) {
    detailLoadedTabsRef.current[order.id] = new Set<DetailTab>();
    setHistory([]);
    setChangeHistory([]);
    setComments([]);
    setMaterials([]);
  ...
  async function loadOrderDetails(orderId: string, force = false) {
    const tab = detailTab;
    if (tab === "summary" || tab === "installation") {
      setDetailLoading(false);
      return;
    }
    const loaded = detailLoadedTabsRef.current[orderId] || new Set<DetailTab>();
    if (!force && loaded.has(tab)) return;

    setDetailLoading(true);
    setDetailError("");
    let detailsError: { message: string } | null = null;

    if (tab === "history" || tab === "production") {
      const [historyResult, changeResult] = await Promise.all([
        supabase
          ...
          .limit(200),
        tab === "history"
          ? supabase.from("order_change_history").select("id,field_name,old_value,new_value,change_group,created_at,changed_by").eq("order_id", orderId).order("created_at", { ascending: false }).limit(200)
          : Promise.resolve({ data: [], error: null }),
      ]);
      detailsError = historyResult.error || changeResult.error;
      if (!detailsError) {
        setHistory((historyResult.data || []) as unknown as HistoryEntry[]);
        if (tab === "history") setChangeHistory((changeResult.data || []) as OrderChangeEntry[]);
      }
      ...
      if (!detailsError) setComments((result.data || []) as unknown as CommentEntry[]);
      ...
      const result = await supabase
        ...
        .limit(250);
      detailsError = result.error;
      if (!detailsError) setMaterials((result.data || []) as OrderMaterial[]);
    } else if (tab === "checklist") {
      const result = await supabase
        .from("order_checklist_items")
        .select("id,order_id,label,category,completed,position,completed_at,created_at")
        .eq("order_id", orderId)
        .order("position")
        .limit(250);
      detailsError = result.error;
      if (!detailsError) setChecklist((result.data || []) as ChecklistItem[]);
    } else if (tab === "files") {
      const result = await supabase
        .from("order_files")
        ...
        .limit(250);
      detailsError = result.error;
      if (!detailsError) setOrderFiles((result.data || []) as OrderFileEntry[]);
    }
  ...
  async function deleteMaterial(material: OrderMaterial) {
    ...
    if (material.purchase_activity_id) {
      const { error: activityDeleteError } = await supabase.from("activities").update({ deleted_at: new Date().toISOString(), deleted_by: user?.id || null }).eq("id", material.purchase_activity_id);
    ...
    const { error: materialError } = await supabase.from("order_materials").update({ deleted_at: new Date().toISOString(), deleted_by: user?.id || null }).eq("id", material.id);
  ...
  async function createDefaultChecklist() {
    ...
    else { await loadOrderDetails(modal.id, true); showNotice("Checklist padrão criado."); }
    setWorkspaceBusy(false);
  }
  ...
  async function addChecklistItem(event: FormEvent<HTMLFormElement>) {
    ...
    else { formElement.reset(); await loadOrderDetails(modal.id, true); }
    setWorkspaceBusy(false);
  }
  ...
  async function toggleChecklistItem(item: ChecklistItem) {
    ...
    else if (modal && modal !== "new") await loadOrderDetails(modal.id, true);
    setWorkspaceBusy(false);
  }
  ...
  async function reopenCompletedOrder(event: FormEvent<HTMLFormElement>) {
    ...
    const nextOrder = reopenedOrder
      ? { ...reopenOrderTarget, ...reopenedOrder } as Order
      : { ...reopenOrderTarget, status: "waiting" as DbStatus, completed_at: null, blocked: false };
    setOrders((current) => current.some((order) => order.id === nextOrder.id)
  ...
  async function bulkUpdateOrders(orderIds: string[], patch: { consultant_name?: string | null; priority?: Priority }) {
    ...
    const { data: updated, error: updateError } = await supabase
      ...
      .in("id", orderIds)
      .select("id,consultant_name,priority,updated_at");
    if (updateError) {
      setError(`Não foi possível atualizar os pedidos selecionados: ${updateError.message}`);
      return false;
    }
    const description = normalizedPatch.consultant_name !== undefined
      ? `Responsável alterado em lote para ${normalizedPatch.consultant_name || "não definido"}.`
      : `Prioridade alterada em lote para ${normalizedPatch.priority || "normal"}.`;
    await supabase.from("order_history").insert(orderIds.map((orderId) => ({
      order_id: orderId,
    ...
    setOrders((current) => current.map((order) => updatedMap.has(order.id) ? { ...order, ...updatedMap.get(order.id) } : order));
    showNotice(`${orderIds.length} pedido(s) atualizado(s).`);
    return true;
  }
  ...
  async function changeUserRole(profile: Profile, nextRole: AppRole) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/users/manage", {
        ...
        body: JSON.stringify({ user_id: profile.id, email: profile.email, action: "change_role", role: nextRole }),
      });
      const payload = await response.json().catch(() => ({})) as { ok?: boolean; user?: Profile; message?: string; error?: string };
      if (!response.ok || !payload.ok || !payload.user) throw new Error(payload.error || "Não foi possível alterar o nível do usuário.");
      ...
      setError(roleError instanceof Error ? roleError.message : "Não foi possível alterar o nível do usuário.");
    } finally {
  ...
  async function manageUser(profile: Profile, action: "activate" | "deactivate" | "resend_invite" | "cancel_invite") {
    ...
    try {
      ...
      const response = await fetch("/api/admin/users/manage", {
        method: "POST",
        headers: { authorization: `Bearer ${accessToken}`, "content-type": "application/json" },
        body: JSON.stringify({ user_id: profile.id, email: profile.email, action }),
      });
      const payload = await response.json().catch(() => ({})) as { ok?: boolean; user?: Profile; message?: string; error?: string };
      if (!response.ok || !payload.ok || !payload.user) throw new Error(payload.error || "Não foi possível alterar o usuário.");
      setProfiles((current) => current.map((item) => item.id === profile.id ? payload.user as Profile : item));
      showNotice(payload.message || "Usuário atualizado.");
    } catch (manageError) {
      setError(manageError instanceof Error ? manageError.message : "Não foi possível alterar o usuário.");
    } finally {
      setProfileBusyId(null);
    }
  }
  ...
  return <main className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`} data-active-view={activeView}>
    ...
    <GlobalSearch open={globalSearchOpen} orders={orders} clients={clients} onClose={() => setGlobalSearchOpen(false)} onOpenOrder={openOrder} onNavigate={navigateTo} />
    <PendingCenter open={pendingCenterOpen} orders={orders} onClose={() => setPendingCenterOpen(false)} onNavigate={navigateTo} />
              ...
              return <div className="os-table-row material-availability-row" key={material.id}>
                ...
                <div className="material-purchase-state">
                  {material.availability === "available"
                    ? <><span className="material-status available">● Disponível</span>{Number(material.received_quantity || 0) > 0 && <small>Recebido: {Number(material.received_quantity).toLocaleString("pt-BR", { maximumFractionDigits: 3 })} {material.unit}</small>}</>
                    : <><span className={`material-status purchase ${material.purchase_status || "pending"}`}>● {purchaseStatus || "Pendente"}</span><small>{Number(material.received_quantity || 0) > 0 ? `Recebido parcialmente: ${Number(material.received_quantity).toLocaleString("pt-BR", { maximumFractionDigits: 3 })} ${material.unit}` : "Prazo definido pela configuração de Compras"}</small></>}
      ...
      <div className="modal user-invite-modal" role="dialog" aria-modal="true" aria-labelledby="new-user-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <form className="user-invite-form" onSubmit={createUser}>
          ...
          <div className="form-grid">
            ...
            <label className="wide">Nível de acesso<select name="role" defaultValue="viewer"><option value="viewer">Visualizador — consulta e comenta</option><option value="production">Produção — movimenta e atualiza</option><option value="manager">Gerente — operação e gestão</option><option value="admin">Administrador — acesso completo</option></select></label>
          </div>
